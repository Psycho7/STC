...
- Per ruling R12: do NOT attempt braid separation with the narrower box (the containment arithmetic forbids it at any width); do not re-open the tier-1b' half-reach cap; resolve the tier-1c cap asymmetry (apply the same containment-bound cap or document with measurement).
...
**Close-out state.** Census 30 / 88 (21 deep) / 47 / 11 at first recording -> 18 / 70 (18 deep) / 39 / 0 here. Gates at close-out: typecheck, lint and vitest (1430 pass / 1 skip) clean; the e2e failure set is the Task 0 baseline exactly (6 failures: geometry-audit multi6 RAW, inputs-panel 180/235/280/342, raw-and-transport:171); all ten placement-shots goldens deleted, regenerated one per invocation and re-verified green - a determinism check, not regression evidence: deleting a golden removes the value the run would have compared against. Across the seven pre-existing audit tables the campaign moved exactly one pre-existing cell, the R13 UP move.
