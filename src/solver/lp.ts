 ...
 *
 * The draw commonly saturates its cap, and a float must not be allowed to
 * round onto some nearby rational instead, so a primal inside the snap window
 * of the cap is snapped onto the cap as an exact Fraction.
 *
 * The tolerance is the historical radius of this snap: absolute 1e-6 at unit
 * scale, relative to the cap above it.
 */
export function snapDraw(primal: number, cap: Fraction): Fraction {
  const capValue = cap.valueOf();
  if (Math.abs(primal - capValue) <= Math.max(SNAP_REL, SNAP_REL * capValue)) {
    return cap;
  }
...
function extractResult(args: ExtractArgs): LpResult {
  ...
  for (const it of items) {
    ...
    if (v <= RATE_ZERO) continue;
    draws.set(it.id, snapDraw(v, cap));
  }
