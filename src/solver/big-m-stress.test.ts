...
// Deficit-dominated + tight cost-cap stress case.
// Satisfiable part: a: M -> F; M produced by b1 (R) and b2 (S), both raw -> the
...
// with X having no producer -> a large deficit (1e9 weight) dominates pass 1.
// The stress: does the deficit domination corrupt the chain solution or swamp
// the lex tie-break? Expected (uncorrupted): x_a=2, x_b1=2, x_b2=0, deficit on X.
...
describe("big-M numerical conditioning", () => {
  it("deficit domination does not corrupt the satisfiable sub-solution or the lex tie-break", () => {
    ...
    // Satisfiable chain still exact under the 1e9-dominated objective.
    expect(r.rates.get("a")!.equals(new Fraction(2))).toBe(true);

    // Lex tie-break still deterministic (b1 over b2) despite the swamped cost-cap.
