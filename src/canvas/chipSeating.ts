...
import { divergenceDotsByIndex, faninDotsByIndex } from "./junctionDots";
...
export function deconflictChipAnchors(
  ...
  // The junction dots of plain item edges: the divergence dot of a declined
  // fan-out and the convergence dot of an all-far fan-in (junctionDots.ts).
  const fanoutJunctionByIndex = divergenceDotsByIndex(edges, itemPtsById);
  const faninJunctionByIndex = faninDotsByIndex(edges, targetYById);
