...
const SAVED_PNG = resolve(
  "/tmp/claude-1000/-home-rins-workspace-STC-workspace-STC/40c36345-04e0-4e2e-9d4e-25824abb520e/scratchpad/export-samples/default.png",
);
...
test("the export button downloads the plan canvas as a framed PNG", async ({
  ...
  mkdirSync(dirname(SAVED_PNG), { recursive: true });
  await download.saveAs(SAVED_PNG);
  ...
  const bytes = await readFile(SAVED_PNG);
