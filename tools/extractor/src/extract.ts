...
// Checked-in ledger of event cohorts (generated by tools/event-cohorts). The
// extractor inverts it into an item-id -> cohort lookup and stamps Item.event /
// Recipe.event from it; see tagEventCohorts.
const EVENT_COHORTS_PATH = resolve(REPO_ROOT, "data/aef/event-cohorts.json");
...
async function main(opts: { write?: boolean } = {}): Promise<ExtractResult> {
  ...
  // The cohort stamp has to run after the collapse (which rewrites stoich
  // refs, so the tagged set must reflect the final rows) and before the raw
  // classifier / referential check below.
  tagEventCohorts({ items, recipes }, eventCohorts);
  ...
  if (write) {
    ...
    const eventItems = items.filter((i) => i.event !== undefined).length;
    const eventRecipes = recipes.filter((r) => r.event !== undefined).length;
    ...
    console.log(
      `  items=${items.length} machines=${machines.length} transports=${transports.length}` +
        ...
        ` taggedEventItems=${eventItems} taggedEventRecipes=${eventRecipes}`,
    );
...
// item ids) into an item-id -> cohort lookup for tagEventCohorts. The ledger
// is generated ahead of the extract (tools/event-cohorts), so a malformed file
// fails here rather than mid-stamp.
async function loadEventCohorts(): Promise<Record<string, string>> {
  const ledger = (await Bun.file(EVENT_COHORTS_PATH).json()) as {
    cohorts?: Record<string, unknown[]>;
  };
  const entries = Object.entries(ledger.cohorts ?? {});
  if (entries.length === 0) {
    throw new Error(`${EVENT_COHORTS_PATH} carries no cohorts`);
  }
  const cohortsByItem: Record<string, string> = {};
  for (const [cohort, ids] of entries) {
    for (const id of ids) {
      if (typeof id !== "string") {
        throw new Error(
          `${EVENT_COHORTS_PATH} cohort ${cohort} lists a non-string id`,
        );
      }
      cohortsByItem[id] = cohort;
    }
  }
  return cohortsByItem;
}

// Stamp the event cohort on every event-bound row, in place. An event item is
// one the ledger names; its cohort is the v<major>.<minor> of the AKEData game
// version that first shipped it (the vendor snapshot only carries the current
// cohort, so ledger ids absent from the pack - a whole retired cohort - are
// fine and simply stamp nothing). A recipe is event-bound when an event item
// sits on its `in` or `out` (never `catalyst`: the transmuter charge is
// machine state, not event content) and inherits that item's cohort.
//
// Two build errors guard the ledger against a vendor snapshot bump: an
// activity-category item the ledger does not name would ship untagged and
// slip past every cohort filter, and a recipe drawing event items from two
// cohorts cannot be assigned one at all.
export function tagEventCohorts(
  pack: {
    items: Item[];
    recipes: Recipe[];
  },
  cohortsByItem: Record<string, string>,
): void {
  for (const item of pack.items) {
    const cohort = cohortsByItem[item.id];
    if (cohort !== undefined) {
      item.event = cohort;
    } else if (item.category === "activity") {
      throw new Error(
        `activity item ${item.id} is missing from the event-cohorts ledger`,
      );
    }
  }
  ...
  for (const r of pack.recipes) {
    ...
    for (const s of [...r.in, ...r.out]) {
      const cohort = cohortsByItem[s.item];
      if (cohort !== undefined) cohorts.add(cohort);
    }
    if (cohorts.size === 0) continue;
    if (cohorts.size > 1) {
      throw new Error(
        `recipe ${r.id} mixes event items from cohorts ${[...cohorts].sort().join(", ")}`,
      );
    }
    r.event = [...cohorts][0]!;
  }
...
async function buildI18nSidecar(
  ...
  for (const locale of LOCALES) {
    ...
    // collapse dropped; strip them so the coverage check matches the emitted
    // pack. Key on droppedRecipes (recipe ids) rather than droppedItems (item
    // ids) so a future substitution whose identity recipe id differs from the
    // item id still produces a clean i18n sidecar.
...
function splitLocale(
  ...
  for (const [id, name] of Object.entries(raw.items)) {
    if (sets.machineIds.has(id)) machines[id] = name;
      ...
      // a collapsed synthetic item or its machine. Neither appears in the
      // pack, so drop the orphan key silently.
      continue;
    } else
